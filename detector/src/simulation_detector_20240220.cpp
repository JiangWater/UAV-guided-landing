...
detectorNode::detectorNode()
{
	...
	nh.param<double>("MpcParam/w_P_xy", w_P_xy, 200);
	nh.param<double>("MpcParam/w_P_z", w_P_z, 50);
	nh.param<double>("MpcParam/w_V_xy", w_V_xy, 20);
	nh.param<double>("MpcParam/w_V_z", w_V_z, 0);
	nh.param<double>("MpcParam/w_q", w_q, 0);
	nh.param<double>("MpcParam/w_P_xy_n", w_P_xy_n, 200);
	nh.param<double>("MpcParam/w_P_z_n", w_P_z_n, 50);
	nh.param<double>("MpcParam/w_V_xy_n", w_V_xy_n, 20);
	...
	detector_sub = nh.subscribe<aruco_msgs::Detector>("/aruco_single/detector", 25, &detectorNode::detector_cb, this);
...
void detectorNode::MPC_control()
{
	...
	if (last_flag == 1)
	{
		...
		x0 << Relative_pos(0), Relative_pos(1), -Relative_pos(2), Relative_dpos(0), Relative_dpos(1), -Relative_dpos(2);

		// xRef << Pos_target(0), Pos_target(1), -Pos_target(2), 0, 0, 0, 1, 0, 0, 0;

		xRef << Pos_target(0), Pos_target(1), -Pos_target(2), 0, 0, 0;
	...
	if (last_flag == 1)
	{
		...
		mpc->solve(x0, xRef, accel_ugv, q_ugv, omega_ugv);
		...
		thrust_sp = mpc_result[0] * mass;
		//  ROS_INFO("thrust_sp:%f", thrust_sp);
		//  ROS_INFO("=====================================");
	...
	*/
	...
	if (last_flag == 1)
	{
		...
		// accel_sp(0) = ctr(0);
		// accel_sp(1) = ctr(1);
		accel_sp(2) = g + mpc_result[2];

		// !!!!!!!!!!  Euler_fcu来自飞控 在反馈回路引入了延迟
		// !!!!!!!!!!  尝试用ugv的姿态和检测的相对姿态来代替
		/*
		...
		cout << endl;
		cout << "Euler_ugv(0): " << Euler_ugv(0) << endl;
		cout << "Euler_ugv(1): " << Euler_ugv(1) << endl;
		cout << "Euler_ugv(2): " << Euler_ugv(2) << endl;
		cout << endl;
		cout << "rpy.x: " << rpy.x << endl;
		cout << "rpy.y: " << rpy.y << endl;
		...
		*/
		...
		thrust_sp = accel_sp(2) / (std::cos(Euler_ugv(2) + rpy.z) * std::cos(Euler_ugv(2) + rpy.z));

		thrust_sp_normalize = thrust_sp * hover_throttle / g;
		thrust_sp_normalize = limit(thrust_sp_normalize, 0.25, 0.6);

		// attitude_sp(2) = attitude_sp(2) + 0.5 * (0 - rpy.z);
		attitude_sp(2) = Euler_ugv(2) + rpy.z - 5 * rpy.z;
	...
	// cout << "body_rate: " << detector_uav_cmd.body_rate.x << ", " << detector_uav_cmd.body_rate.y << ", " << detector_uav_cmd.body_rate.z << endl;
	// cout << "roll, pitch, yaw: " << rpy.x << ", " << rpy.y << ", " << rpy.z << endl;
	...
	*/
	...
	if (file_created == 1 && data_record == 1)
	{
		oFile << current_time << "," << id << "," << flag << "," << Relative_pos(0) << "," << Relative_pos(1) << "," << Relative_pos(2) << "," << Pos_target(2) - Relative_pos(2) << "," << Relative_dpos(0) << "," << Relative_dpos(1) << "," << Relative_dpos(2) << "," << Relative_accel_sp(0) << "," << Relative_accel_sp(1) << "," << Relative_accel_sp(2) << "," << solve_time << "," << endl;
	}
}

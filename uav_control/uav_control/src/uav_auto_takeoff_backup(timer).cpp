...
ros::Subscriber detector_vel_sub;
...
void cmd_pub_timer_cb(const ros::TimerEvent &event)
{
    // while (ros::ok())
    //{
    //     ros::spinOnce();
...
int main(int argc, char **argv)
{
    ...
    detector_vel_sub = nh.subscribe<aruco_msgs::Detector_vel>("/aruco_single/detector_vel", 50, detector_vel_cb);
    state_sub = nh.subscribe<mavros_msgs::State>("mavros/state", 10, mav_state_cb);

    command_pub = nh.advertise<prometheus_msgs::ControlCommand>("/prometheus/control_command", 50);
    attitude_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/mavros/setpoint_raw/attitude", 50);
    cmd_pub_timer = nh.createTimer(ros::Duration(1 / rate_hz), cmd_pub_timer_cb);

...
double max_accel_xy;   // 最大水平加速度
double max_accel_z;    // 最大垂直加速度
...
Eigen::Matrix<double, 9, 1> Lmpc_x0;
Eigen::Matrix<double, 9, 1> Lmpc_xRef;
Eigen::Matrix<double, 9, 1> Lmpc_predicted_x0;
...
Eigen::Matrix<double, 13, 1> Nmpc_x_dot(const Eigen::Matrix<double, 13, 1> &x, const Eigen::Vector4d &q_ugv, const Eigen::Vector3d &omega_ugv, const Eigen::Vector3d &alpha_ugv, const Eigen::Vector3d &accel_ugv_w, const double &T, const Eigen::Vector3d &body_rate)
{
    Eigen::Matrix<double, 13, 1> x_dot;
    x_dot << x(6),
        x(7),
        x(8),
        x(0),
        x(1),
        x(2),
        (omega_ugv(2) * omega_ugv(2) + omega_ugv(1) * omega_ugv(1)) * x(0) + (-omega_ugv(0) * omega_ugv(1) + alpha_ugv(2)) * x(1) + (-omega_ugv(0) * omega_ugv(2) - alpha_ugv(1)) * x(2) + 2 * (omega_ugv(2) * x(7) - omega_ugv(1) * x(8)) + 2 * (x(10) * x(12) + x(9) * x(11)) * T - 2 * (q_ugv(1) * q_ugv(3) - q_ugv(0) * q_ugv(2)) * 9.8 - accel_ugv_w(0),
        (-omega_ugv(0) * omega_ugv(1) - alpha_ugv(2)) * x(0) + (omega_ugv(0) * omega_ugv(0) + omega_ugv(2) * omega_ugv(2)) * x(1) + (-omega_ugv(1) * omega_ugv(2) + alpha_ugv(0)) * x(2) + 2 * (-omega_ugv(2) * x(6) + omega_ugv(0) * x(8)) + 2 * (x(11) * x(12) - x(9) * x(10)) * T - 2 * (q_ugv(2) * q_ugv(3) + q_ugv(0) * q_ugv(1)) * 9.8 - accel_ugv_w(1),
        (-omega_ugv(0) * omega_ugv(2) + alpha_ugv(1)) * x(0) + (-omega_ugv(1) * omega_ugv(2) - alpha_ugv(0)) * x(1) + (omega_ugv(0) * omega_ugv(0) + omega_ugv(1) * omega_ugv(1)) * x(2) + 2 * (omega_ugv(1) * x(6) - omega_ugv(0) * x(7)) + (1 - 2 * x(10) * x(10) - 2 * x(11) * x(11)) * T - (1 - 2 * q_ugv(1) * q_ugv(1) - 2 * q_ugv(2) * q_ugv(2)) * 9.8 - accel_ugv_w(2),
        0.5 * (-body_rate(0) * x(10) - body_rate(1) * x(11) - body_rate(2) * x(12) + omega_ugv(0) * x(10) + omega_ugv(1) * x(11) + omega_ugv(2) * x(12)),
        0.5 * (body_rate(0) * x(9) + body_rate(2) * x(11) - body_rate(1) * x(12) - omega_ugv(0) * x(9) + omega_ugv(2) * x(11) - omega_ugv(1) * x(12)),
        0.5 * (body_rate(1) * x(9) - body_rate(2) * x(10) + body_rate(0) * x(12) - omega_ugv(1) * x(9) - omega_ugv(2) * x(10) + omega_ugv(0) * x(12)),
        0.5 * (body_rate(2) * x(9) + body_rate(1) * x(10) - body_rate(0) * x(11) - omega_ugv(2) * x(9) + omega_ugv(1) * x(10) - omega_ugv(0) * x(11));

    return x_dot;
}

Eigen::Matrix<double, 13, 1> Nmpc_rk4_predictor(const Eigen::Matrix<double, 13, 1> &x0, const double &dt, const Eigen::Vector4d &q_ugv, const Eigen::Vector3d &omega_ugv, const Eigen::Vector3d &alpha_ugv, const Eigen::Vector3d &accel_ugv_w, const double &T, const Eigen::Vector3d &body_rate)
{
    Eigen::Matrix<double, 13, 1> Nmpc_predicted_x0;
    Eigen::Matrix<double, 13, 1> k1;
    Eigen::Matrix<double, 13, 1> k2;
    Eigen::Matrix<double, 13, 1> k3;
    Eigen::Matrix<double, 13, 1> k4;
    k1 = Nmpc_x_dot(x0, q_ugv, omega_ugv, alpha_ugv, accel_ugv_w, T, body_rate);
    k2 = Nmpc_x_dot(x0 + k1 * 0.5 * dt, q_ugv, omega_ugv, alpha_ugv, accel_ugv_w, T, body_rate);
    k3 = Nmpc_x_dot(x0 + k2 * 0.5 * dt, q_ugv, omega_ugv, alpha_ugv, accel_ugv_w, T, body_rate);
    k4 = Nmpc_x_dot(x0 + k3 * dt, q_ugv, omega_ugv, alpha_ugv, accel_ugv_w, T, body_rate);
    Nmpc_predicted_x0 = x0 + (k1 + 2 * k2 + 2 * k3 + k4) * dt / 6.0;
    return Nmpc_predicted_x0;
}

Eigen::Matrix<double, 9, 1> Lmpc_x_dot(const Eigen::Matrix<double, 9, 1> &x0, const Eigen::Vector3d &u)
{
    Eigen::Matrix<double, 9, 1> x_dot;
    x_dot << x0(6),
        x0(7),
        x0(8),
        x0(0),
...
Eigen::Matrix<double, 9, 1> Lmpc_rk4_predictor(const Eigen::Matrix<double, 9, 1> &x0, const double &dt, const Eigen::Vector3d &u)
{
    Eigen::Matrix<double, 9, 1> Lmpc_predicted_x0;
    ...
    k1 = Lmpc_x_dot(x0, u);
    k2 = Lmpc_x_dot(x0 + k1 * 0.5 * dt, u);
    k3 = Lmpc_x_dot(x0 + k2 * 0.5 * dt, u);
    k4 = Lmpc_x_dot(x0 + k3 * dt, u);
    Lmpc_predicted_x0 = x0 + (k1 + 2 * k2 + 2 * k3 + k4) * dt / 6.0;
    return Lmpc_predicted_x0;
}

Eigen::Matrix<double, 9, 1> Lmpc_one_step_predictor(const Eigen::Matrix<double, 9, 1> &x0, const double &dt, const Eigen::Vector3d &u)
{
    Eigen::Matrix<double, 9, 1> Lmpc_predicted_x0;
    Lmpc_predicted_x0 << x0(0) + x0(6) * dt,
        x0(1) + x0(7) * dt,
        x0(2) + x0(8) * dt,
        x0(3) + x0(0) * dt,
        x0(4) + x0(1) * dt,

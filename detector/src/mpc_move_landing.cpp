...
bool MPC::setNmpcSolver()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        */
        ...
        SX k1 = dynamic_equation(input).at(0);
        input[0] += dt / 2.0 * k1;
        SX k2 = dynamic_equation(input).at(0);
        input[0] += dt / 2.0 * k2;
        SX k3 = dynamic_equation(input).at(0);
        input[0] += dt * k3;
        SX k4 = dynamic_equation(input).at(0);
        SX X_next = X(all, i) + (k1 + 2 * k2 + 2 * k3 + k4) * dt / 6.0;
        */
